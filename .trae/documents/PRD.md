...
- **主色**：暖纸色页面底 `#f6f5f0`，靛蓝主色 `#4f46e5`，深靛强调色 `#4338ca`，暖灰边框 `#e6e1d5`
- **辅助色**：语义色取自 Tailwind 调色板 —— 成功绿（emerald）、警示 / 数据过期黄（amber）、危险操作红（rose / orange）、中性灰（slate）
- **按钮风格**：圆角（6-8px），主要按钮靛蓝底白字，次要按钮描边无填充，危险操作用红色描边
- **字体**：自托管 `Space Grotesk`（可变字体，latin 子集，无衬线）用于标题与正文，`JetBrains Mono` 用于数字（强调数据感），中文回退系统字体（PingFang SC / Noto Sans SC / 微软雅黑）
...
| 页面名称 | 模块名称 | UI 元素 |
|-----------|-------------|-------------|
| 决策工作台 | 任务列表侧栏 | 暖灰背景，任务卡片含商品名+推荐摘要，激活项靛蓝左竖线 |
| 决策工作台 | 商品信息头 | 大号无衬线标题，类别 tag，决策偏好 segmented control，备注输入框 |
